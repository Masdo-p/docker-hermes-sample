...
1. Harus lewat git repo — deploy_project nggak bisa build Dockerfile, jadi wajib create_app_from_repo/deploy_private_repo + build_pack=dockercompose, compose=True. Compose me-mount volume ke /opt/data supaya data hermes tidak hilang tiap redeploy.
2. Auth pakai _PASSWORD plaintext, bukan _PASSWORD_HASH — ini yang bikin "Invalid username or password" kemarin.
3. HEALTHCHECK di Dockerfile — Coolify tahu kapan hermes siap (compose redeploy tetap sempat 502 ~1-2 menit saat boot).
